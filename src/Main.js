...
const steps = ['The Backstory', 'Enter Text', 'Keyword Report'];
...
export default function Main() {
  ...
  async function getKeywords(){
    const encodedText = encodeURIComponent(state.text);
    try {
      const resp = await fetch(
        `https://api.zacharyjklein.com/keyme/?text=${encodedText}`
      );
      return resp.json()
    } catch (err) {
      console.log(err);
  ...
  async function handleSubmit() {
    ...
    setState(
      { ...state, activeStep: state.activeStep + 1, isLoading: false, keywords: keywords }
    );
  };
